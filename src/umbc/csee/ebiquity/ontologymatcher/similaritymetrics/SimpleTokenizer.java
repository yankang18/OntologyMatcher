package umbc.csee.ebiquity.ontologymatcher.similaritymetrics;
 ...
 * 
 * <p>
 * The delimiter can be used in one of two ways, depending on how the
 * singleDelimiter flag is set:
 * <ul>
 ...
 * 
 * <p>
 * The total number of tokens in the text is equal to the number of delimeters
 * found plus one. An empty token is returned when:
 * <ul>
...
public class SimpleTokenizer {
	 ...
	 * 
	 * <p>
	 * If the singleDelmiter flag is true, then the delimiter string is used as
	 * a single delimiter. If the flag is false, the each character in the
	 * delimiter is treated as a delimiter.
	 * 
	 * @param text
	 *            a string to be tokenized.
	 * @param delimiter
	 ...
	 *            treat each character as a delimiter.
	 */
	public SimpleTokenizer(String text, String delimiter,
	...
	/**
	 * Set the delimiter(s) used to parse the text. The delimiter can be reset
	...
	/**
	 * Tests if there are more tokens available from this tokenizer. A
	 * subsequent call to <tt>nextToken</tt> will return a token.
	 * 
	...
	/**
	 * Returns the nth token from the current position of this tokenizer. This
	 * is equivalent to advancing n-1 tokens and returning the nth token.
	 * 
	 * @param tokenCount
	 *            the relative position of the token requested
